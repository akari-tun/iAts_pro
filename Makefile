...
RELEASE_VARIANTS			:= $(VALID_VARIANTS)

# By default, all platforms support all variants. 
# Platforms that support only some variants can include a variants file with one supported variant per line
...
HAL_COMMON					:= $(ROOT)/lib/hal-common/
VPATH          				:= $(ROOT)/lib/hal-esp32/
VPATH          				:= $(HAL_COMMON)
...
help:
		...
		@echo "Variant targets are $(VALID_TARGETS)"
		@echo "Use TARGET=<target> make to compile a target"
		@echo "To flash a target, use TARGET=<target> PORT=<port> make flash"
		@echo "On macOS, Linux and Unix-like systems port must be the full port path e.g. /dev/tty.SLAB-USBtoUART"
...
show-targets:
		@echo $(LD_LIBRARY_PATH)
		@echo "Valid target are $(VALID_TARGETS)"
